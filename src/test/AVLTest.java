...
public class AVLTest {
    ...
    private final Filme[] filmes = {
        new Filme(5, "F", 2020, 5),
        new Filme(3, "D", 2020, 5),
        new Filme(4, "E", 2020, 5),
    ...
    public void testBalance() {
        try {
            ...
            assertEquals(3, tree.root().getID());
            assertEquals(0, tree.treeMinimum().getFilme().getID());
            assertEquals(5, tree.treeMaximum().getFilme().getID());
        } catch (Exception e) {
            assertEquals("A arvore está vazia", e.getMessage());
        }
    }
